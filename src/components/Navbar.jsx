...
const Navbar = () => {
  ...
  const address = "0xabcdefghijklmnopqrstwxyz";
                ...
                <ul className='mb-4'>
                  {navlinks.map((link) => (
                    <li key={link.name} className={`flex p-4 ${isActive === link.name && 'bg-[#3a3a43]'}`} onClick={() => {
                      ...
                      <p className={`ml-[20px] font-epilogue font-semibold text-[14px] ${isActive === link.name ? 'text-[#1dc071]' : 'text-[#8080191]'}`}>{link.name}</p>
                    </li>
                  ))}
                </ul>
